...
TAG=$(git rev-parse --abbrev-ref HEAD)-$(git log --pretty=format:'%h' -n 1)"-j9"
...
docker build --file=Dockerfile.j9 --tag=${IMAGE}:${TAG} .
source ./down.bash
docker run -d --name=${CONTAINER_NAME} --hostname=${CONTAINER_NAME} -eLC_ALL=en_US.UTF-8 -e"${JAVA_TOOL_OPTIONS}" ${PORTS_TO_PUBLISH} ${IMAGE}:${TAG}
