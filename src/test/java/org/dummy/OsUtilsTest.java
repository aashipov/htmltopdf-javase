...
class OsUtilsTest {
    ...
    void whyOsUtils() throws IOException, InterruptedException {
        ...
        if (isWindows()) {
            ...
            String err = inputStreamToStringJdk9Plus(process.getErrorStream(), DEFAULT_CHARSET);
            String out = inputStreamToStringJdk9Plus(process.getInputStream(), DEFAULT_CHARSET);
        ...
        if (isLinux()) {
            ...
            Assertions.assertThrows(
                    IOException.class,
                    () -> {
                        inputStreamToStringJdk9Plus(process.getInputStream(), DEFAULT_CHARSET);
                    },
                    "Stream closed");
            Assertions.assertThrows(
                    IOException.class,
                    () -> {
                        inputStreamToStringJdk9Plus(process.getErrorStream(), DEFAULT_CHARSET);
                    },
                    "Stream closed");
        }
    }
